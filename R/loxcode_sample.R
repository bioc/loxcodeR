...
load_prob_files <- function(path) {
    size_idx <- 0:4
    rec_dist <- seq_len(15)
    l <- lapply(size_idx, function(s) {
        vapply(rec_dist, function(r) {
            paste(path, paste0('size_', s, '_rec', r), sep = '/')
        }, FUN.VALUE = character(length(rec_dist)))
    })
...
setMethod("retrieve_prob_ensemble", "loxcode_sample", function(x) {
    ...
    for (i in sizes) {
        ...
        for (j in seq_len(nrow(r))) {
            ...
            probs <-
                probs + r$prob[j] * loxcodeR::retrieve_prob(x@decode@data[mask,]$id,
