...
merge_sample <- function(s1, s2) {
    m <-
        merge(
            s1@decode@data,
            s2@decode@data,
            by = c('code', 'size', 'is_valid', 'id', 'dist_orig'),
            all = TRUE
        )
    ...
    s@decode@saturation <-
        list(s1@decode@saturation, s2@decode@saturation)
    s@decode@read_ids <-
        list(s1@decode@read_ids, s2@decode@read_ids)
...
setMethod('merge_by', 'loxcode_experiment', function(x, by) {
    ...
    x_merged <-
        new(
            "loxcode_experiment",
            name = paste(x@name, 'merge_by', by, sep = '_'),
            dir = x@dir
        )
...
# #' collapse_old(x, count_matrix, collapse = c("SampleID", "Treatment"), name = "CollapsedCounts", union = TRUE, average = FALSE)
# setGeneric("collapse_old", function(x,
#                                     count_matrix,
#                                     collapse,
#                                     name,
#                                     union = TRUE,
#                                     average = FALSE) {
#     standardGeneric("collapse_old")
# })
#
# #' @rdname collapse_old
...
#     U = as.data.frame(unique(S[, collapse]), stringsAsFactors = FALSE)
...
# #' collapse_selection_old(lox, s, c, i, n, union = TRUE, average = FALSE)
#
# setGeneric("collapse_selection_old", function(lox,
#                                               s = "all_samples",
#                                               c = "all_codes",
#                                               i,
#                                               n = NULL,
#                                               union = TRUE,
#                                               average = FALSE) {
#     standardGeneric("collapse_selection_old")
# })
#
# #' @rdname collapse_selection_old
# setMethod("collapse_selection_old", "loxcode_experiment", function(lox,
#                                                                    s = "all_samples",
#                                                                    c = "all_codes",
#                                                                    i,
#                                                                    n = NULL,
...
#     }
#
#     cols = data.frame(row.names = seq(seq_len(nrow(counts))))
#     cols[[sample_name]] = NA
