...
#' @return Loxcode experiment object
loxcode_experiment <- setClass("loxcode_experiment",
    representation(name = "character", dir = "character",
        samples = "list", samp_table = "data.frame",
        count_matrixes = "list", code_sets = "list",
        meta = "data.frame", alias = "list"),
    prototype = list(name = "", dir = "", samples = list(),
        samp_table = data.frame(), count_matrixes = list(),
        code_sets = list(), meta = data.frame(),
        alias = list()))
#' CHECK IF IN SHINY
#'
...
shiny_running <- function() {
    ...
    frames <- sys.frames()
    calls <- lapply(sys.calls(), `[[`,
        1)
    call_name <- function(call) if (is.function(call))
        "<closure>" else deparse(call)
    call_names <- vapply(calls, call_name,
        character(1))
    target_call <- grep("^runApp$",
        call_names)
    ...
    isNamespace(namespace_frame) &&
        environmentName(namespace_frame) ==
            "shiny"
}
...
setMethod("load_samples", "loxcode_experiment", function(x,
    full = FALSE, sat = FALSE) {
    x@samples <- lapply(names(x@samples), function(z) {
        print(z)
        if (shiny_running()) {
            shiny::showNotification(paste("Loading",
                z, "..."))
        }
        print(x@samp_table$sample)
        samp_table_sliced <- x@samp_table[match(z,
            x@samp_table$sample), ]
        files <- list.files(x@dir)
        if (length(files[grepl(".fastq$", files)]) >
            0) {
            files <- sort(files[grepl(".fastq$", files)])
            indices <- grepl(x@samples[[z]], files)
            matches <- sort(files[indices])
            print(c(paste0(x@dir, matches[1]), paste0(x@dir,
                matches[2])))
            print("A")
            out <- loxcoder::decode(c(paste0(x@dir,
                matches[1]), paste0(x@dir, matches[2])),
                name = z, meta = samp_table_sliced[,
                  !names(samp_table_sliced) %in% c("sample",
                    "prefix", "min_r1_len", "min_r2_len")],
...
# #' Set names of samples #' #' @param x lox code object #' @param v
# samples names # #' @param value updating loxcode object #' @return
# Names of unique samples #' @export setGeneric('sampnames<-',
# function(x, v){ standardGeneric('sampnames<-')})
# setMethod('sampnames<-', 'loxcode_experiment', function(x, v){
# if(length(v) != length(unique(v))){ stop('Sample names are not
# unique') } names(x@samples) <- v })
...
#' #' Set experiment name
# #' #' @param x loxcode object #' @param v name of sample # #'
# @param value updating loxcode object #' @return name of samples #'
# @export setGeneric('name<-', function(x, v)
# {standardGeneric('name<-')}) setMethod('name<-',
# 'loxcode_experiment', function(x, v){ x@name <- v })
...
#' #experiment
load_from_xlsx <- function(name, s, dir, load = TRUE, full = FALSE,
    sat = FALSE) {
...
# Same as load_from_xlsx but now the parameters s, dir, suffix_R1,
# suffix_R2 are lists
load_from_xlsx_multi <- function(name, s, dir, suffix_R1, suffix_R2,
    load = TRUE, full = FALSE, sat = FALSE) {
    x <- new("loxcode_experiment", name = name, dir = dir)
    return(x)
}
...
merge_sample <- function(s1, s2) {
    ...
    s@decode_stats <- list(s1@decode_stats, s2@decode_stats)
    return(s)
}
...
setMethod("merge_by", "loxcode_experiment", function(x, by) {
    ...
    x_merged <- new("loxcode_experiment", name = paste(x@name,
        "merge_by", by, sep = "_"), dir = x@dir)
...
#' @return count matrix for each sample
...
setMethod("setup_count_matrix_codesets", "loxcode_experiment", function(x) {
    ...
    for (i in x@samp_table$sample) {
        m <- merge(m, loxcoder::sample(x, i)@decode@data, by = "code",
            all = TRUE)
    }
    codes <- m$code[-1]
    # View(m)
    ...
    # all.codes=data.frame(code=row.names(mm),
    # size=rowMeans(ms,na.rm=TRUE), is_valid=rowMeans(mv,na.rm=TRUE),
    # dist_orig=rowMeans(mo,na.rm=TRUE),stringsAsFactors = FALSE)
    # all.codes= rbind(code=row.names(mm),
    # size=rowMeans(ms,na.rm=TRUE), is_valid=rowMeans(mv,na.rm=TRUE),
    # dist_orig=rowMeans(mo,na.rm=TRUE),stringsAsFactors = FALSE)
    # all.codes=data.frame(code=names(mm), size=ms, is_valid=mv,
    # dist_orig=mo)
    all.codes <- data.frame(code = codes, size = merged_ms, is_valid = merged_mv,
        dist_orig = merged_mo, stringsAsFactors = FALSE)
    # View(all.codes)
    valid.codes <- subset(all.codes, is_valid == 1)
    # valid.codes$flp_dist<-
    # loxcoder::min_flip_dist(valid.codes$code, valid.codes$size,
    # valid.codes$is_valid)$d_min
...
#'
#' @param x loxcode object to be updated
#' @return metadata for loxcode experiment
...
#' @return loxcode object with alias
#' @rdname fill_alias
#' @export
#' @examples
#' # Load necessary libraries and data
...
setMethod("fill_alias", "loxcode_experiment", function(x) {
    ...
    for (i in seq_along(x@samples)) {
        row <- data.frame(sample_name = name(x@samples[[i]]),
            alias = paste("Sample", i), stringsAsFactors = FALSE)
...
#' @rdname make_codeset_index
setMethod("make_codeset_index", "loxcode_experiment", function(x, c,
    I, n) {
    new_experiment <- x
    newset <- data.frame()
    for (i in I) {
...
setGeneric("delete_codeset", function(x, n) {
    standardGeneric("delete_codeset")
})
#' @rdname delete_codeset
setMethod("delete_codeset", "loxcode_experiment", function(x,
    n) {
    # always retain all_codes, valid_codes and invalid_codes
    if (n == "all_codes" | n == "valid_codes" | n ==
        "invalid_codes") {
        return(x)
    } else {
        new_experiment <- x
...
#' @rdname delete_count_matrix
setMethod("delete_count_matrix", "loxcode_experiment", function(x,
    c) {
    if (c == "all_samples") {
        return(x)
    } else {
...
#' @rdname rename_sample
setMethod("rename_sample", "loxcode_experiment", function(x, c, n,
    o) {
    # count matrixes
...
# #' #' @param x loxcode_experiment object #' @param count_matrix
# current count_matrix #' @param collapse column names of metadata on
# which to collapse #' @param name name of new count_matrix #' @param
# union boolean, True if barcodes in either should be counted #'
# @param average boolean, True if counts should be averaged instead
# of summed #' @return loxcode_experiment object with new
# count_matrix #' @rdname collapse_old #' @export #' @examples #' #
# Load required packages #' library(loxcoder) #' #' # Example usage
# #' # Assuming x and count_matrix are defined and have the required
# structure #' collapse_old(x, count_matrix, collapse = c('SampleID',
# 'Treatment'), name = 'CollapsedCounts', union = TRUE, average =
# FALSE) setGeneric('collapse_old', function(x, count_matrix,
# collapse, name, union = TRUE, average = FALSE) {
# standardGeneric('collapse_old') }) #' @rdname collapse_old
# setMethod('collapse_old', 'loxcode_experiment', function(x,
# count_matrix, collapse, name, union = TRUE, average = FALSE) { M =
# x@count_matrixes[[count_matrix]] S = get_collapsed_meta(x,
# count_matrix) S = cbind(sample_name = 0, S) S$sample_name =
# row.names(S) row.names(S) = seq(1, nrow(S)) U =
# as.data.frame(unique(S[, collapse]), stringsAsFactors = FALSE) #
# get unique combinations U = na.omit(U) MM = data.frame(row.names =
# row.names(M), stringsAsFactors = FALSE) #new data.frame to store
# results n = array() index = c() for (c in collapse) { index =
# append(index, match(c, names(S))) } #loop over all unique
# combinations for (i in seq_len(nrow(U))) { filter = array() for (j
# in seq_len(nrow(S))) { filter[j] = all(S[j, index] == U[i,]) }
# filter[is.na(filter)] = FALSE if (sum(filter == TRUE) == 0) next
# cols = sum(filter == TRUE) # bardcode in any sample if (cols > 1 &
# union) { rsums = rowSums(M[, filter]) if (average) { MM = cbind(MM,
# (rsums / cols)) } else { MM = cbind(MM, rsums) } } # barcode in all
# samples else if (sum(filter == TRUE) > 1 & !union) { rsums =
# rowSums(M[, filter]) if (average) { MM = cbind(MM, (apply(M[,
# filter] > 0, 1, all) * rsums / cols)) } else { MM = cbind(MM,
# (apply(M[, filter] > 0, 1, all) * rsums)) } } else { MM = cbind(MM,
# M[, filter]) } #or if only one sample n[i] = paste(U[i,], collapse
# = '__') } names(MM) = n is.num <- vapply(MM, is.numeric) MM[is.num]
# <- lapply(MM[is.num], round, 2) x@count_matrixes[[name]] <- MM
# alias = data.frame() for (i in seq_len(ncol(MM))) { row =
# data.frame( sample_name = names(MM)[[i]], alias = paste('Sample',
# i), stringsAsFactors = FALSE ) alias = plyr::rbind.fill(alias, row)
# } x@alias[[name]] = alias return(x) })
...
#' @rdname get_collapsed_meta2
setMethod("get_collapsed_meta2", "loxcode_experiment", function(x,
    s) {
    sample_name <- NULL
    ...
    if (length(col_names) > 0) {
        ...
        for (i in seq_along(col_meta)) {
            row <- data.frame(sample_name = col_names[[i]],
                t(col_meta[[i]]), stringsAsFactors = FALSE)
            df1 <- plyr::rbind.fill(df1, row)
        }
        used <- c()
        for (i in 2:ncol(df1)) {
            for (j in seq_len(ncol(x@meta))) {
                if (!names(x@meta)[[j]] %in% used & all(df1[[i]] %in%
                  x@meta[[j]])) {
...
#' @rdname make_filtered_codeset
setMethod("make_filtered_codeset", "loxcode_experiment", function(x,
    s, c, t, m, n) {
    codeset <- x@code_sets[[c]]
    YY <- filtered_codes_table(x, s, c, t, m)
    x@code_sets[[n]] <- subset(codeset, paste(codeset$size,
        codeset$dist_orig) %in% paste(YY$size, YY$dist_orig))
    return(x)
})
#' rename code set
#'
...
#' @rdname generate_alias
setMethod("generate_alias", "loxcode_experiment", function(lox,
    s = "all_samples", meta) {
    x <- lox
    aliases <- x@alias[[s]]
    ...
    for (i in seq_len(nrow(aliases))) {
        ...
        alias <- paste(alias, as.character(counts[[alias]]),
            sep = "_")
        aliases$alias[[i]] <- alias
    }
    x@alias[[s]] <- aliases
...
setGeneric("merge_experiments", function(lox1, lox2, name = NULL) {
    standardGeneric("merge_experiments")
})
#' @rdname merge_experiments
setMethod("merge_experiments", "loxcode_experiment", function(lox1, lox2,
    name = NULL) {
    plyr <- NULL
    ...
    names(lox1@samples) <- plyr::revalue(names(lox1@samples), rename1,
        warn_missing = FALSE)
    names(lox2@samples) <- plyr::revalue(names(lox2@samples), rename2,
        warn_missing = FALSE)
    ...
    lox1@samp_table$sample <- plyr::revalue(lox1@samp_table$sample, rename1,
        warn_missing = FALSE)
    lox2@samp_table$sample <- plyr::revalue(lox2@samp_table$sample, rename2,
        warn_missing = FALSE)
    new@samp_table <- plyr::rbind.fill(lox1@samp_table, lox2@samp_table)
    # count_matrixes slot
    for (i in seq_along(lox1@count_matrixes)) {
        names(lox1@count_matrixes[[i]]) <- plyr::revalue(names(lox1@count_matrixes[[i]]),
            rename1, warn_missing = FALSE)
    }
    for (i in seq_along(lox2@count_matrixes)) {
        names(lox2@count_matrixes[[i]]) <- plyr::revalue(names(lox2@count_matrixes[[i]]),
            rename2, warn_missing = FALSE)
    }
    ...
    new@code_sets[["all_codes"]] <- unique(rbind.fill(codes1[["all_codes"]],
        codes2[["all_codes"]]))
    new@code_sets[[paste0("all_codes_", lox1@name)]] <- codes1[["all_codes"]]
    new@code_sets[[paste0("all_codes_", lox2@name)]] <- codes2[["all_codes"]]
    new@code_sets[["valid_codes"]] <- unique(rbind.fill(codes1[["valid_codes"]],
        codes2[["valid_codes"]]))
    ...
    new@code_sets[["invalid_codes"]] <- unique(rbind.fill(codes1[["invalid_codes"]],
        codes2[["invalid_codes"]]))
    ...
    others1 <- codes1[!names(codes1) %in% c("all_codes", "valid_codes",
        "invalid_codes")]
    others2 <- codes2[!names(codes2) %in% c("all_codes", "valid_codes",
        "invalid_codes")]
    ...
    # meta slot
    lox1@meta$sample_name <- plyr::revalue(lox1@meta$sample_name, rename1,
        warn_missing = FALSE)
    lox2@meta$sample_name <- plyr::revalue(lox2@meta$sample_name, rename2,
        warn_missing = FALSE)
    ...
    for (i in seq_along(lox1@alias)) {
        lox1@alias[[i]]$sample_name <- plyr::revalue(lox1@alias[[i]]$sample_name,
            rename1, warn_missing = FALSE)
    }
    for (i in seq_along(lox2@alias)) {
        lox2@alias[[i]]$sample_name <- plyr::revalue(lox2@alias[[i]]$sample_name,
            rename2, warn_missing = FALSE)
    }
    all_aliases <- data.frame(sample_name = names(all_samples)[!names(all_samples) ==
...
# #' Collapse selected samples (old version) #' #' @param lox
# loxcode_experiment object #' @param s sample set/ count_matrix #'
# @param c code set #' @param i indices of samples to collapse #'
# @param n new sample name #' @param union boolean: use union or
# intersection of samples #' @param average boolean: use average of
# sum of counts #' @return new loxcode_experiment object #' @rdname
# collapse_selection_old #' @export #' @examples #' # Load required
# packages #' library(loxcoder) #' #' # Example usage #' # Assuming
# lox, s, c, i, n, union, and average are defined and have the
# required structure #' collapse_selection_old(lox, s, c, i, n, union
# = TRUE, average = FALSE) setGeneric('collapse_selection_old',
# function(lox, s = 'all_samples', c = 'all_codes', i, n = NULL,
# union = TRUE, average = FALSE) {
# standardGeneric('collapse_selection_old') }) #' @rdname
# collapse_selection_old setMethod('collapse_selection_old',
# 'loxcode_experiment', function(lox, s = 'all_samples', c =
# 'all_codes', i, n = NULL, union = TRUE, average = FALSE) { counts =
# lox@count_matrixes[[s]] counts = subset(counts, row.names(counts)
# %in% lox@code_sets[[c]]$code) meta = get_collapsed_meta(lox, s)[i,
# ] sample_name = c() for (k in seq_len(ncol(meta))) { if
# (sum(is.na(meta[, k])) == 0 & dim(table(meta[, k])) == 1) {
# sample_name = c(sample_name, meta[1, k]) } } sample_name =
# paste0(sample_name, collapse = '__') if (sample_name == '') {
# sample_name = paste0('NA', sum(grepl('NA', names(counts)))) } cols
# = data.frame(row.names = seq(seq_len(nrow(counts))))
# cols[[sample_name]] = NA for (k in seq_len(nrow(counts))) { if
# (union == TRUE & average == TRUE) { cols[k, 1] = sum(counts[k, i])
# / length(i) } if (union == TRUE & average == FALSE) { cols[k, 1] =
# sum(counts[k, i]) } if (union == FALSE & average == TRUE) { cols[k,
# 1] = sum(counts[k, i]) * as.numeric(sum(counts[k, i] == 0) == 0) /
# length(i) } if (union == FALSE & average == FALSE) { cols[k, 1] =
# sum(counts[k, i]) * as.numeric(sum(counts[k, i] == 0) == 0) } }
# counts = cbind(counts, cols) counts[, i] = NULL
# lox@count_matrixes[[s]] = counts # update the aliases aliases =
# data.frame('sample_name' = names(counts), 'alias' = NA)
# aliases$alias = paste0('Sample ', seq(seq_len(nrow(aliases))))
# lox@alias[[s]] = aliases return (lox) })
