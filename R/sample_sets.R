...
setMethod("summary_table", "loxcode_experiment", function(lox, sample_set="all_samples"){
  ...
  table = merge(table, metadata, all=TRUE, by=c("sample_name"), sort=FALSE)
  table$`Barcode Count` = sapply(samples, function(x) nrow(x@decode@data))
  table$`Invalid Count` = sapply(samples, function(x) nrow(subset(x@decode@data, is_valid==FALSE)))
  table$`Number of Reads` = sapply(samples, function(x) x@decode_stats$tot_reads)
  table$`Max Complexity` = sapply(samples, function(x) max(x@decode@data$dist_orig, na.rm = TRUE))
  table$`Consensus Filtered` = sapply(samples, function(x) x@decode_stats$consensus_filtered)
  table$`Percent Filtered` = sapply(samples, function(x) round(100*x@decode_stats$consensus_filtered/x@decode_stats$tot_reads, 2))
  return (table)
})
...
fillSetAliases <- function(lox, count_matrix) {
  ...
  aliases = data.frame(sample_name = names(lox@count_matrixes[[count_matrix]]),
                       alias = "",
                       stringsAsFactors = FALSE)
  aliases$alias = c(sapply(1:nrow(aliases), function(x) paste("Sample", x)))
...
setMethod("collapse_selection", "loxcode_experiment", function(lox, count_matrix="all_samples", code_set="all_codes", index, name=NULL, union=TRUE, average=FALSE){
  ...
  if (length(index) == 1) {
    ...
    counts[[name]] = (rowSums(counts[ , samples_list]) * matrixStats::rowProds(as.matrix(counts)[, samples_list]>0))
  } else if (!union & average) {
    counts[[name]] = rowSums(counts[ , samples_list]) * matrixStats::rowProds(as.matrix(counts)[, samples_list]>0) / length(samples_list)
  }
  lox@count_matrixes[[count_matrix]] = counts

  # add to aliases
  aliases = lox@alias[[count_matrix]]
  row = data.frame("sample_name" = name, "alias" = paste("Sample", nrow(aliases)+1))
...
setMethod("collapse", "loxcode_experiment", function(lox, count_matrix, collapse, name, union=TRUE, average=FALSE) {
  ...
  for (i in 1:length(track)) {
    if (length(track[[i]]) == 1) new_count_matrix[[i]] = counts[[track[[i]][1]]]
    else if (union & !average) new_count_matrix[[i]] = rowSums(counts[ , track[[i]]])
    else if (union & average) new_count_matrix[[i]] = rowSums(counts[ , track[[i]]]) / length(track[[i]])
    else if (!union & !average) new_count_matrix[[i]] = (rowSums(counts[ , track[[i]]]) * matrixStats::rowProds(as.matrix(counts)[, track[[i]]]>0))
    else if (!union & average) new_count_matrix[[i]] = rowSums(counts[ , track[[i]]]) * matrixStats::rowProds(as.matrix(counts)[, track[[i]]]>0) / length(track[[i]])
  }
...
setMethod("make_count_matrix", "loxcode_experiment", function(lox,
  ...
  # handle null values
  if (is.null(lox) || is.null(indices)) { return ()}
